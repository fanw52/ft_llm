...
# @st.cache_resource
# def get_model():
#     model_path = "/data/pretrained_models/stablelm-tuned-alpha-7b"
#     tokenizer = AutoTokenizer.from_pretrained(model_path)
#     model = AutoModelForCausalLM.from_pretrained(model_path,load_in_8bit=False,torch_dtype=torch.float16, device_map='auto')
#     model.half().cuda()
#     model = model.eval()
...
#             in_text_decode = tokenizer.decode(inputs["input_ids"][0])
#             answer = response.replace(in_text_decode, "").replace("\nEND", "").strip()
#
